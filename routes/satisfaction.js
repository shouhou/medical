...
router.post('/satisfactionAdd/upload', function(req, res) {
...
router.post('/satisfactionList/batchRPC', function(req, res) {
    var dataStr = '';
    if (dataStr = req.param('dataStr')) {
        ...
        for (var i in datas) {
            ...
            if (data.length > 1) {
                var keys = ['patient_code', 'visit_no', 'satisfaction', 'satisfaction_content', 'visit_date_time', 'record_date_time', 'visit_type'];
                var json = {};
                for (var j in data) {
                    json[keys[j]] = data[j];
                }
                var func = async.apply(function(json, callback) {
                    satisfaction.getOut(json, function(out) {
                        soap.sendReq(out, function(data) {
                            data['patient_code'] = json['patient_code'];
                            callback(null, data);
                        });
                    });
                }, json);
