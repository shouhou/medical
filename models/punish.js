...
module.exports = {
    getPunishById: function(id, callback) {
        conn.query('select ' + util.querySql(json) + ' from wp_users where id = ?', [id], function(rows, fields) {
            callback(rows);
        });
    },

    getAllPunish: function(callback) {
        conn.query('select ' + util.querySql(json) + ' from wp_users', function(rows, fields) {
            callback(rows);
        });
    }
};
